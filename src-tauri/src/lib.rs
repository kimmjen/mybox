...
fn config_path() -> Option<PathBuf> {
    let home = env::var("HOME").ok()?;
    Some(PathBuf::from(home).join(MYBOX_DIR).join("config.json"))
}
...
#[tauri::command]
fn get_output_folder(app_handle: tauri::AppHandle) -> String {
    let cfg = read_config();
    if let Some(folder) = cfg["output_folder"].as_str() {
...
#[tauri::command]
fn save_output_folder(path: String) -> Result<(), String> {
    if !Path::new(&path).is_dir() {
        return Err("유효하지 않은 폴더 경로입니다.".into());
    }
    let mut cfg = read_config();
    cfg["output_folder"] = serde_json::json!(path);
    write_config(&cfg);
    Ok(())
}
...
fn check_dependencies() -> Vec<DepStatus> {
    ...
    let pillow = find_python3()
        .map(|p| pillow_installed(&p))
        .unwrap_or(false);
    ...
    vec![
        ...
        DepStatus {
            name: "nvm".into(),
            label: "nvm (Node Version Manager)".into(),
            installed: nvm,
        },
        DepStatus {
            name: "node".into(),
            label: format!("Node.js ≥ {} (via nvm)", MIN_NODE),
            installed: node,
        },
        DepStatus {
            name: "playwright".into(),
            label: "Playwright + Chromium".into(),
...
fn install_dependencies(app_handle: tauri::AppHandle) -> Result<String, String> {
    std::thread::spawn(move || {
        ...
        if brew_path().is_none() {
            ...
            emit(
                "brew",
                "Terminal에서 Homebrew 설치를 완료한 후 [다시 확인]을 눌러주세요.",
                "needs_interaction",
            );
            return;
        }
        ...
        if nvm_sh_path().is_none() {
            ...
            match out {
                ...
                Err(e) => {
                    emit("nvm", &format!("nvm 설치 실패: {}", e), "error");
                    return;
                }
            }
        } else {
        ...
        if node_via_nvm().is_none() {
                ...
                .spawn()
            {
                Ok(c) => c,
                Err(e) => {
                    emit("node", &format!("Node.js 설치 실패: {}", e), "error");
                    return;
                }
            };
            ...
            match child.wait() {
                ...
                Ok(_) => {
                    emit("node", "Node.js 설치에 실패했습니다.", "error");
                    return;
                }
                Err(e) => {
                    emit("node", &format!("Node.js 설치 실패: {}", e), "error");
        ...
        if !playwright_package_installed() {
                ...
                .spawn()
            {
                Ok(c) => c,
                Err(e) => {
                    emit("playwright", &format!("npm install 실패: {}", e), "error");
                    return;
                }
            };
            if let Some(stdout) = child.stdout.take() {
                for line in BufReader::new(stdout).lines().flatten() {
                    let l = line.trim().to_string();
                    if !l.is_empty() {
                        emit("playwright", &l, "running");
                    }
                }
            }
            match child.wait() {
                Ok(s) if s.success() => {
                    emit("playwright", "playwright 패키지 설치 완료", "running")
                }
                _ => {
                    emit("playwright", "playwright 패키지 설치 실패", "error");
                    return;
                }
            }
        }
        ...
        if !playwright_browser_installed() {
                ...
                .spawn()
            {
                Ok(c) => c,
                Err(e) => {
                    emit("playwright", &format!("Chromium 설치 실패: {}", e), "error");
                    return;
                }
            };
            if let Some(stdout) = child.stdout.take() {
            ...
            match child.wait() {
                Ok(s) if s.success() => {
                    emit("playwright", "Playwright + Chromium 설치 완료 ✓", "done")
                }
                _ => {
                    emit("playwright", "Chromium 설치 실패", "error");
                    return;
                }
            }
        } else {
        ...
        if pyenv_bin().is_none() {
            ...
            match out {
                ...
                Err(e) => {
                    emit("pyenv", &format!("pyenv 설치 실패: {}", e), "error");
                    return;
                }
            }
        } else {
        ...
        if python_in_venv().is_none() {
            ...
            emit(
                "python",
                &format!("Python {} 설치 중... (5~15분 소요)", PYTHON_VER),
                "running",
            );
                ...
                .spawn()
            {
                Ok(c) => c,
                Err(e) => {
                    emit("python", &format!("Python 설치 실패: {}", e), "error");
                    return;
                }
            };
            ...
            match child.wait() {
                Ok(s) if s.success() => {}
                Ok(_) => {
                    emit("python", "Python 설치에 실패했습니다.", "error");
                    return;
                }
                Err(e) => {
                    emit("python", &format!("Python 설치 실패: {}", e), "error");
            ...
            match out {
                ...
                Err(e) => {
                    emit("python", &format!("가상환경 생성 실패: {}", e), "error");
                    return;
                }
            }
        } else {
        ...
        let python = match python_in_venv() {
            Some(p) => p,
            None => {
                emit("pillow", "Python 경로를 찾을 수 없습니다.", "error");
                return;
            }
        };
        ...
        if !pillow_installed(&python) {
            ...
            match out {
                ...
                Err(e) => {
                    emit("pillow", &format!("Pillow 설치 실패: {}", e), "error");
                    return;
                }
            }
        } else {
...
fn start_extraction(
    ...
    let downloads_path = if !output_folder.is_empty() && Path::new(&output_folder).is_dir() {
        output_folder.clone()
    } else {
    ...
    std::thread::spawn(move || {
        ...
        emit_status(
            "compiling",
            "PDF 병합 엔진 (Python/Pillow) 가동 중...",
            None,
            None,
        );

        let compile_title = if custom_filename.is_empty() {
            doc_title.clone()
        } else {
